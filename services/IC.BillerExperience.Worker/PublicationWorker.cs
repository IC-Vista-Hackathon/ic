    ...
    ILogger<PublicationWorker> logger) : BackgroundService
{
    ...
    [LoggerMessage(1000, LogLevel.Information, "IC Biller Experience publication worker is ready; trace {TraceId}")]
    private static partial void LogWorkerReady(ILogger logger, string? traceId);

    [LoggerMessage(1001, LogLevel.Information, "IC Biller Experience publication worker stopped; trace {TraceId}")]
    private static partial void LogWorkerStopped(ILogger logger, string? traceId);

    [LoggerMessage(1900, LogLevel.Error, "IC Biller Experience publication worker failed; trace {TraceId}")]
