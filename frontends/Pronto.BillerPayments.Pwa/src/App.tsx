...
export function App() {
  ...
  async function pay() { if (!invoice || !provider) return; setBusy(true); setError(''); trackEvent('pwa.payment_submitted', { method, scheduled: schedulesPayment, autopay_opt_in: autoPay, paperless_opt_in: paperless }); let completed: PaymentReceipt; try { completed = await provider.pay({ invoiceId: invoice.id, method, autoPay, paperless, scheduledFor: schedulesPayment ? invoice.dueDate : undefined, payerName, payerEmail, accountNumber: invoice.accountNumber }); } catch (caught) { setError(`Submit payment: ${errorMessage(caught)}`); logError('pwa.payment.failed', caught, { method, scheduled: schedulesPayment }); trackEvent('pwa.payment_failed', { method, scheduled: schedulesPayment, error_category: categorizeError(caught) }); setBusy(false); return; } setReceipt(completed); setStep('complete'); logEvent('pwa.payment.completed', { method, scheduled: schedulesPayment, autopay_opt_in: autoPay, paperless_opt_in: paperless }); trackEvent('pwa.payment_completed', { method, scheduled: schedulesPayment, autopay_opt_in: autoPay, paperless_opt_in: paperless }); try { if (completed.payerAccountId) { const profile = await provider.findPayer(invoice.accountNumber); setPayer(profile); if (profile) setPayments(await provider.getPayments(profile.payer_id)); } } catch (caught) { logError('pwa.payment.history_refresh_failed', caught, { method }); } finally { setBusy(false); } }
    ...
    <main><Intro eyebrow="Account services" heading={page === 'payment' ? config.content.heading : page === 'history' ? 'Account history' : 'Communication preferences'} subheading={page === 'payment' ? config.content.introduction : `Manage services for ${config.brand.display_name}.`} />{error && <div className="alert" role="alert" data-testid="error">{error}</div>}
      {page === 'payment' && <>
        ...
        {step === 'method' && invoice && <section className="card"><Bill invoice={invoice}/><h2>Choose how to pay</h2><div className="choices">{acceptedMethods.includes('card') && <button data-testid="method-card" className={method === 'card' ? 'selected' : 'option'} onClick={() => selectMethod('card')}>Card <small>{quoteFeeText(quotes.card, invoice.amountCents)}</small></button>}{acceptedMethods.includes('ach') && <button data-testid="method-ach" className={method === 'ach' ? 'selected' : 'option'} onClick={() => selectMethod('ach')}>Bank Account <small>{quoteFeeText(quotes.ach, invoice.amountCents)}</small></button>}</div>{acceptedMethods.filter(value => value !== 'card' && value !== 'ach').length > 0 && <div className="method-chips">Also accepts {acceptedMethods.filter(value => value !== 'card' && value !== 'ach').map(humanize).join(' · ')}</div>}
          ...
          <button data-testid="review-submit" onClick={openReview} disabled={acceptedMethods.length === 0 || ((autoPay || paperless) && (!payerName || !payerEmail))}>Review Payment</button></section>}
