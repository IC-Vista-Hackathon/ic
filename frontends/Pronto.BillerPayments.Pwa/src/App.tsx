...
export function App() {
  ...
  async function savePreferences() { if (!provider || !payer) return; setBusy(true); setError(''); setPrefsSaved(false); try { const updated = await provider.updatePreferences(payer.payer_id, { ...payer.preferences, autopay: autoPay, paperless, channels: payer.preferences.channels.length ? payer.preferences.channels : ['email'], payment_day: autoPay ? (payer.preferences.payment_day ?? 15) : payer.preferences.payment_day }); setPayer({ ...payer, preferences: updated }); setPrefsSaved(true); logEvent('pwa.preferences.saved', { outcome: 'saved' }); trackEvent('pwa.preferences_saved', { outcome: 'saved' }); } catch (caught) { setError(`Save preferences: ${errorMessage(caught)}`); logError('pwa.preferences.save_failed', caught); trackEvent('pwa.preferences_saved', { outcome: 'failed', error_category: categorizeError(caught) }); } finally { setBusy(false); } }
  function navigate(next: Page) { setPrefsSaved(false); setPage(next); }
  ...
  function changeAutoPay(enabled: boolean) { setPrefsSaved(false); setAutoPay(enabled); trackEvent('pwa.autopay_changed', { enabled }); }
  function changePaperless(enabled: boolean) { setPrefsSaved(false); setPaperless(enabled); trackEvent('pwa.paperless_changed', { enabled }); }
  ...
  const batchPayButtonLabel = cartTotalCents !== undefined ? `Pay ${money(cartTotalCents)}` : 'Quote unavailable';
...
function Bill({ invoice }: { invoice: Invoice }) {
  ...
  return <div className={`bill${paid ? ' bill-paid' : ''}`}>
    <div className="bill-main">
      <div className="bill-desc">
        {invoice.type && <span className="bill-type" data-testid="bill-type">{invoice.type}</span>}
        <span>{invoice.description}</span>
        <small>{paid ? 'Paid' : `Due ${new Date(invoice.dueDate).toLocaleDateString()}`}</small>
      </div>
      <div className="bill-right">
        {paid
          ? <span className="status-dot status-green" data-testid="status-dot" title="Paid" aria-label="Paid" />
