const CACHE = 'biller-payments-v2';
...
self.addEventListener('install', event => event.waitUntil(caches.open(CACHE).then(cache => cache.addAll([self.registration.scope, new URL('config.json', self.registration.scope).href]))));
