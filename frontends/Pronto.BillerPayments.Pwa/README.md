...
The payer assistant is **opt-in and off by default** — set `VITE_PAYER_ASSISTANT=true` at build/dev
time to enable it. With it off, the payer page renders exactly the standard manual flow (no panel,
no `payer-chat` call). It is also only shown for the single-invoice flow, not the multi-invoice cart.

When enabled, after a bill is looked up the app shows a "Payment assistant" panel above the manual
payment flow. It calls the Biller Experience API's `POST /billers/{billerId}/payer-chat`, which runs the
...
VITE_CONFIG_URL=/pay/config.local.json VITE_PAYER_ASSISTANT=true npm run dev   # in this directory
