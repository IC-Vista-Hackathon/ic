...
export class DemoPaymentExperienceProvider implements PaymentExperienceProvider {
  ...
  pay(request: PaymentRequest) { return observed('pwa.payment.demo', async () => { await new Promise(resolve => setTimeout(resolve, 650)); return { confirmation: `DEMO-${randomId().slice(0, 8).toUpperCase()}`, amountCents: 12842, feeCents: request.method === 'card' ? 250 : 95 }; }); }
}
