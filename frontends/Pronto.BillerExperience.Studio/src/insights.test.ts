...
describe('initBrowserTelemetry', () => {
  it('stays disabled when no connection string is configured', async () => {
    ...
    expect(() => trackEvent('studio.session_started')).not.toThrow();
  });
  ...
  it('flushes events queued before init, with context attached', async () => {
    trackEvent('studio.session_started');
    trackEvent('studio.not_allowlisted');
    stubConfigResponse({ connection_string: 'InstrumentationKey=x', sampling_percentage: 100 });
    await initBrowserTelemetry();

    const [instance] = instances;
    expect(instance.events).toHaveLength(1);
    expect(instance.events[0].name).toBe('studio.session_started');
...
describe('trackEvent', () => {
  it('sanitizes properties before handing them to the SDK', async () => {
    ...
    trackEvent('studio.preview_opened', {
      device: 'mobile',
      biller_id: 'b1c2d3e4-5678-4abc-9def-0123456789ab',
      display_name: 'City of Vista',
      chat_text: 'looks great',
    });

    const [event] = instances[0].events;
    expect(event.properties.device).toBe('mobile');
    expect(event.properties.biller_id).toBe('b1c2d3e4-5678-4abc-9def-0123456789ab');
    expect(event.properties).not.toHaveProperty('display_name');
    expect(event.properties).not.toHaveProperty('chat_text');
  });
  ...
  it('drops non-allowlisted events entirely', async () => {
    ...
    trackEvent('studio.chat_message', { text: 'hello' });
...
describe('enforceAllowlist (SDK pipeline last line of defense)', () => {
  ...
  it('re-scrubs properties and clears measurements on allowed events', () => {
    const item = {
      baseType: 'EventData',
      baseData: {
        name: 'studio.publish_failed',
        properties: { error_category: 'http_5xx', website: 'https://leak.example.com', message: 'raw error' },
        measurements: { amount: 12.34 },
      },
    };
    expect(enforceAllowlist(item as never)).toBe(true);
    expect(item.baseData.properties).toEqual({ error_category: 'http_5xx' });
...
describe('flowId', () => {
  ...
  it('persists via sessionStorage when available', () => {
    ...
    const first = flowId();
    expect(store.get('pronto.studio.flow_id')).toBe(first);
    resetBrowserTelemetryForTests();
    expect(flowId()).toBe(first);
  });
});
