...
// Application Insights browser telemetry for the Biller Studio, configured at runtime from the
// Biller Experience API so the connection string is never baked into the frontend build. The
// Studio is same-origin behind the gateway, so it reuses the shared GET /api/public/telemetry
// config endpoint (see TelemetryController.cs) — no second endpoint. Only allowlisted semantic
// events leave the page (see telemetryPolicy.ts); the SDK's automatic collection is switched off
// so it cannot capture URLs with biller data, request bodies, or raw exception text. A stable
// per-load trace_id rides on every event so browser telemetry joins server-side request traces.
...
const FLOW_STORAGE_KEY = 'pronto.studio.flow_id';

// One trace id per page load, shared by every browser event so they correlate in Application
// Insights. hex32 to match the W3C trace-id shape used for server correlation.
const sessionTraceId = randomId().replaceAll('-', '');
...
/** Stable, non-PII flow id for this onboarding session; survives reloads within the tab session. */
 ...
 * string is configured. Resolves false (and leaves telemetry disabled) on any failure — billers
...
function contextProperties(): Record<string, TelemetryValue> {
  return { flow_id: flowId(), trace_id: sessionTraceId };
}
